...
public class MainActivity extends AppCompatActivity {
    ...
    private class TsunamiAsyncTask extends AsyncTask<URL, Void, Event> {
         ...
         */
        // InputStream contains a stream of incoming bytes w/o knowing the datatype.
        private String readFromStream(InputStream inputStream) throws IOException {
            // StringBuilder is mutable unlike String. We use StringBuilder since we are building the string overtime with the incoming data so we would like to modify the same data.
            StringBuilder output = new StringBuilder();
            if (inputStream != null) {
                InputStreamReader inputStreamReader = new InputStreamReader(inputStream, Charset.forName("UTF-8"));

                //BufferReader class converts the incoming bytes into something meaningful like text.
